...
class RNNAttentionDecoder(nn.Module):
    def __init__(
        ...
        self.dropout = nn.Dropout(p=dropout)
        self.umbedding = nn.Linear(2 * hid_dim, vocab_size)
        self.max_length = max_length
    
    def forward(self, image_features, gt_captions=None):
        device = image_features.device
        encoder_hidden, encoder_outputs = image_features[:, 1:, :], \
            image_features[:, :1, :].permute(1, 0, 2).contiguous()
        ...
        for i in range(self.max_length):
            decoder_output, rnn_state, weights =\
                self.forward_step(decoder_input, rnn_state, encoder_hidden)
    ...
    def forward_step(
        self, decoder_input, rnn_state, encoder_hidden):
        tok_embeds = self.dropout(self.embedding(decoder_input))
            
        rnn_hidden, rnn_state = self.rnn_dec(tok_embeds, rnn_state)
        
        dot_prods = torch.einsum('bmd,bnd->bmn', encoder_hidden, rnn_hidden)
        attn_weights = torch.softmax(dot_prods, dim=1)
        context = torch.einsum('bmd,bmn->bnd', encoder_hidden, attn_weights)
        
        decoder_hidden = torch.cat((context, rnn_hidden), dim=-1)
        decoder_output = self.umbedding(self.dropout(decoder_hidden))

        return decoder_output, rnn_state, attn_weights
